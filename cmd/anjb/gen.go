...
// splitLinkOnlyPackages parses the -linkonly flag value into package import
// paths, dropping empty entries and surrounding spaces.
func splitLinkOnlyPackages(list string) []string {
	var paths []string
...
// genLinkOnlyPkg writes go_linkonlymain.go, which blank-imports the -linkonly
// packages so their cgo //export symbols and init side effects are linked into
// the output library even though no bindings are generated for them.
func genLinkOnlyPkg(pkgPaths []string) {
	w, closer := writer(filepath.Join("src", "anjb", "go_linkonlymain.go"))
	defer closer()
	if err := writeLinkOnlyGo(w, pkgPaths); err != nil {
		errorf("failed to write -linkonly imports: %v", err)
	}
}

func writeLinkOnlyGo(w io.Writer, pkgPaths []string) error {
	var buf bytes.Buffer
	buf.WriteString("// Code generated by anjb. DO NOT EDIT.\n\n")
	buf.WriteString("// Packages listed with -linkonly are linked for their cgo //export\n")
	buf.WriteString("// symbols and init side effects; no bindings are generated for them.\n")
