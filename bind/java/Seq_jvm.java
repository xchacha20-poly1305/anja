...
public class Seq {
	...
	private static boolean loadBundledLibrary(String name) {
		...
		try (InputStream in = Seq.class.getResourceAsStream(resource)) {
			...
			log.fine("failed to load bundled native library " + resource + ": " + e.getMessage());
			return false;
		}
	}

	private static String normalizeOS(String raw) {
		String os = raw.toLowerCase();
		if (os.contains("win")) {
			return "windows";
		}
